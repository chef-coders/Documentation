...
Über den Stift-Button auf der rechten Seite, wird das Nutzer bearbeiten-Fragment geöffnet.
Dieses ermöglichst das Bearbeiten des eigene Benutzer-Profils.

Darunter zeigen sich viele Buttons, mit denen zu den verschiedenen Fragmenten der Main-Activity navigiert werden kann:

\begin{itemize}
	\item Shop-Button: Das Shop-Fragment wird geöffnet.
	Dies ist das zentrale Fragment der Main-Activity, da hier Artikel gekauft werden können.

	\item Einkaufs Historie-Button: Das Einkaufs Historie-Fragment öffnet sich, in welchem alle vergangenen Einkäufe nach Datum und Zeit sortiert aufgelistet werden (siehe Abbildung ~\ref{purchases}).
	...
	\item Statistiken-Button: Wird dieser Button geklickt, öffnet sich das Statistiken-Fragment, welches Statistiken zum Verhalten des Benutzers liefert.

	\item Einstellungen-Button: Über diesen Button kann das Einstellungen-Fragment geöffnet werden.
	In diesem kann ein Dunkel-Modus aktiviert werden.

\end{itemize}

Der unterste Button, der mit dem Label ''Logout-Button´´ versehen ist, hat nichts mit der Navigation zwischen Fragments zu tun.
Bei einem Klick auf diesen Eintrag werden die in den Shared-Preferences gespeicherten Anmeldedaten gelöscht.
Er wird zur Login-Activity weitergeleitet, um sich einen neuen Account zu erstellen.
Auf Grund des Designs des REST-Servers ist eine erneute Anmeldung nicht möglich.

\subsection{Nutzer bearbeiten-Fragment} \label{subsec:edit-user-fragment}

\subsection{Shop-Fragment}\label{subsec:shop-fragment}

Nach erfolgreicher Anmeldung erfolgt eine Weiterleitung auf das Shop-Framgent der Main-Activity.
Dieses zeigt eine Liste der kaufbaren Gegenstände (siehe Abbildung ~\ref{mainscreen}).
...
Wenn alle gewünschten Gegenstände ausgewählt sind, wird durch einen Klick auf den Einkaufswagen-Button das Warenkorb-Fragment geöffnet.
...
Beim Klick auf \glqq yes\grqq{} wird der Kauf getätigt und man wird auf das Shop-Fragment weitergeleitet.
...
Durch einen Klick auf das Kamera-Icon in der oberen, rechten Ecke des Shop-Fragments (siehe Abbildung ~\ref{mainscreen}) öffnet sich die Kamera des Gerätes mit einem eingebauten Barcode-Scanner.
...
\subsection{Shop-Fragment (Zusätzliche Admin Funktionalitäten)} \label{subsec:shop-fragment-admin}
...
Ist der aktuelle Nutzer als Administrator ausgezeichnet, kann er mit dem +-Knopf im oberen Banner des Shop-Fragments neue Gegenstände hinzufügen.
...
\subsection{Einkaufs Historie-Fragment} \label{subsec:purchases-fragment}
...
\subsection{Nutzer-Fragment (Admin Funktionalität)} \label{subsec:user-fragment}

\subsection{Statistiken-Fragment} \label{subsec:statistics-fragment}

\subsection{Einstellungen-Fragment} \label{subsec:settings-fragment}
